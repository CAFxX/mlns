...
pb = ProgressBar.create(:output => STDERR, :format => PBFMT, :title => "Base cases  ", :total => C-1)
cs=[[0]]
(C-1).times {
...
pb = ProgressBar.create(:output => STDERR, :format => PBFMT, :title => "Permutations", :total => cs.length)
pcs=Set.new
cs.each{|c| 
	pcs.merge(c.permutation)
	pb.increment
}
...
pb = ProgressBar.create(:output => STDERR, :format => PBFMT, :title => "Mapping     ", :total => pcs.length)
m={}
pcs.each{|x|
		pb.increment
