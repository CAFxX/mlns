...
#norderings = pcs.map{|x| find_all_indexes(x, sort_indexes(x)) }.uniq.length
nvalidcases = pcs.map{|x| comparison_results(x) }.uniq.length
...
puts "// comparisons=#{(C*C-C)/2}, evals=#{pcs.length}, total cases=#{2**((C*C-C)/2)}, valid cases=#{nvalidcases}, orderings=#{(1..C).reduce(1, :*)}"
...
puts "      default: out = {#{C}{B{1'bx}}}; // #{2**((C*C-C)/2) - nvalidcases} invalid cases" unless nvalidcases == 2**((C*C-C)/2)
puts "    endcase"
puts "  end"
puts "endmodule"